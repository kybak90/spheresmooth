...
```{r}
cyclone_list$row_count[cyclone_list$id == '1515'] ##row count
index = as.numeric(cyclone_list$i[cyclone_list$id == '1515'])
df1[[index]]
...
for(i in 71:81)
{
   data_list[i-1,] = c(as.numeric(df1[[index + (i-1)]][1]), as.numeric(df1[[index + (i-1)]][5])*0.1, as.numeric(df1[[index + (i-1)]][4])*0.1, as.numeric(df1[[index + (i-1)]][3]), as.numeric(df1[[index + (i-1)]][6]), as.numeric(df1[[index + (i-1)]][12]), 
as.numeric(df1[[index + (i-1)]][17]) %/% 10000, as.numeric(df1[[index + (i-1)]][17]) - (10000 * as.numeric(df1[[index + (i-1)]][17]) %/% 10000), as.numeric(df1[[index + (i-1)]][18]), as.numeric(df1[[index + (i-1)]][19]) %/% 10000, as.numeric(df1[[index + (i-1)]][19]) - (10000 * as.numeric(df1[[index + (i-1)]][19]) %/% 10000), as.numeric(df1[[index + (i-1)]][20]))
}
```
...
```{r}
data_list = data_list[-c(70:81),]
time_range = range(data_list$time)

spherical_data = matrix(0, ncol = 3, nrow = nrow(data_list))

spherical_data[, 1] = (data_list[, 1] - time_range[1]) / (time_range[2] - time_range[1])
...
dimension = 60
t = spherical_data[, 1]
lambda_seq = exp(seq(log(1e-07), log(1), length = 50))
initial_knots = s_knots_quantile(t, dimension = dimension)
# initial_knots1 = s_knots_quantile(t[1:31], dimension = dim1)
# initial_knots1[dim1] = initial_knots1[dim1] - 1e-05
# initial_knots2 = s_knots_quantile(t[35:69], dimension = dim2)
...
pch3d(fit[[best_index]]$control_points, col = "red", cex = 0.1, pch = 17)
test = spherical_to_cartesian(c(90-33,124) * pi /180)
test2 = spherical_to_cartesian(c(90-33.35,130)* pi /180)
text3d(test, texts = 'korea')
text3d(test2, texts = 'japan')
...
# Save RData
```{r}
save.image(file = "fit1_goni.RData")
```
